    ...
    <li class="my_order_content_box">
        ...
        <div class="order_goods_box">
          ...
          <div class="order_btn_box">
            <a @click="ReminderShipment(item.order_no,item.order_type)" class="baseColor">提醒发货</a>
...
<script>
import { GetPersonalOrderList ,reminderShipment,Getordernum} from '../../../api/index.js'
export default {
  data () {
    return {
      orderList:[],//订单列表
  ...
  methods: {
    ...
    getOrderNUm(){
      Getordernum().then(res=>{
        if(res.code == 0){
          this.totalPage = parseInt(res.data.count2)
        }
      })
    },
     //获取当前页码
