  ...
  <div class="accountDiv banxin">
    ...
    <div class="accountInfo">
      <div class="accountInfoTop">
        <h3>
          <img src="../../../assets/common/fail.png" alt="">
          当前订单状态：等待买家付款。
        </h3>
      </div>
      <div class="accountInfoBottom">
        <p class="prompt">温馨提醒:</p>
        <p class="promptInfo">如果您不想购买，可以<b @click="getOrderInfoCancel">取消订单</b></p>
        <a href="javascript:void(0);" @click="submitOrder">立即付款</a>
      </div>
    </div>
    <!-- 订单信息 -->
    <div class="infoDiv">
    ...
    <div class="infoBottom">
      <p class="fr">应付款: <b>￥{{ OrderProductList.price_total }}</b></p>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getOrderInfoCancel(){
      ...
      orderInfoCancel(params).then(res=>{
        if(res.code == 0){
          ...
          this.$router.push({name: 'orderInfoDel',query:param})//已取消
        }
      })
    },
    // 去付款
    submitOrder(){
      let params = {
        order_no : this.orderNo,
        price_total: this.OrderProductList.price_total,
        form:1,
      }
      if(this.orderType == 1){
