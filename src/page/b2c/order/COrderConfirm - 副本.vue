      ...
      <div class="order_ok">
            ...
            <table>
              ...
              <tbody>
                <tr v-for="item in productList" :key="item.index">
                  ...
                  <td>{{ item.price }}</td>
                  <td>
                    <el-input-number v-model="item.num" @change="handleChange" :min="1"  :max="parseInt(item.changeNum)" size="mini" class="el_input_num"></el-input-number>
                  </td>
                  <td>{{ item.transExpenses }}</td>
                  <td>
                    {{ item.price*item.num + parseFloat(item.transExpenses) | strip }}
                  </td>
                </tr>
              </tbody>
            </table>
            ...
            <table>
              ...
              <tbody>
                <tr v-for="items in item.goods" :key="items.index">
                  ...
                  <td>{{ items.price }}</td>
                  <td>
                    <el-input-number v-model="items.num" @change="handleChange" :min="1"  :max="parseInt(items.changeNum)" size="mini" class="el_input_num"></el-input-number>
                  </td>
                  <td >
                ...
                <tr class="tr_bottom">
                  <td colspan="3" class="couponTd">
                    <span>优惠券</span>
                    <div v-for="coup in item.coupon" :key="coup.index" :class="coup.checked == true?'couponActive':''"  @click="couponChange(item,index,coup.couponId)">
                      <p class="price" v-if="coup.couponId ==1">不使用</p>
                      <p class="time" v-if="coup.couponId ==1"></p>
                      <p class="price" v-if="coup.couponId !=1">{{ coup.name }}</p>
                      <p class="time" v-if="coup.couponId !=1">有效期:{{ coup.end_time }}</p>
                    </div>
                  </td>
        ...
        <div class="total_message">
          <div class="total_message_box">
            <p v-if="this.$route.query.from == 0"><span>应付款:</span> ¥ {{ allPrice | strip }}</p>
...
export default {
  ...
  methods: {
    ...
    // },
    couponChange(item,itemIndex,couponId){
    ...
    handleChange (value) {
      this.num1 = value
      this.allPrice = this.stripFun(this.num1 * this.$route.query.price)+parseFloat(this.$route.query.transExpenses)
    ...
    everyShopMoney(index){
      ...
      list.forEach(function(item, index, arr) {
          everyStoreMoney += that.stripFun(parseFloat(item.num) * parseFloat(item.price)-parseFloat(couponPrice))
      });
      return that.stripFun(everyStoreMoney+parseFloat(transExpenses))
    },
    //购物车总金额
    cartNum(){
      var that = this
      ...
      for(var i =0, len = that.cartProductList.length; i<len; i++){
        ...
        that.allShopPrice = that.allShopPrice + parseFloat(transExpenses)
      }
    },
    // 提交订单
    submitOrder(){
