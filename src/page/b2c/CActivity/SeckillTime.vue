  ...
  <div>
    ...
    <div class="goodSaleDiv">
      ...
      <div class="goods_Div banxin">
        <div class="time_banner">
          <div class="swiper-container timeBanner">
            <div class="swiper-wrapper">
              <div class="swiper-slide" v-for="item in SecillTop" :key="item.index" :class="{ active: isActiveNUm == item.scene_id }"
                @click="toggleBranner(item.scene_id)">
                <div>{{ item.seckill_time }}</div>
                <div>{{ item.seckill_title }}</div>
                <div v-if="item.is_begin == 1" class="swiper-countDown">距结束:<count-down :o_time="item.over_time"></count-down>
                </div>
        ...
        <ul>
          <!-- 进行中 -->
          <li v-for="item in secillDataList" :key="item.index">
            <div class="goods_img">
              <img :src="item.thumb">
            </div>
            ...
            <div class="endTime-Div">
              ...
              <router-link :to="'/one/b2c/CBodyCommon/CProductDetail/'+item.activity_type+'/'+item.product_id+'/'+item.activity_id"
                tag="a">
                <el-button v-if="item.seckill_status == 1" class="goods_buy_btn">立即购买</el-button>
              </router-link>
              <el-button v-if="item.seckill_status == 2" class="goods_buy_btn partake">已参与</el-button>
              <el-button v-if="item.seckill_status == -1" class="goods_buy_btn partake">即将开始</el-button>
              <el-button v-if="item.seckill_status == 0" class="goods_buy_btn partake">已抢光</el-button>
            </div>
          </li>
        </ul>
        <div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 获取数据
      getb2cpersonalSeckill() {
        let params = {
          scene_id: this.isActiveNUm,
          page: this.page,
          limit: this.limit
        };
        b2cpersonalSeckill(params).then(res => {
          if (res.code == 0) {
...
<style scoped lang="scss">
  .goodSaleDiv {
    .img_box {
      background: url(../../../assets/common/activity/seckillTimeBG.png) no-repeat top center;
      width: 100%;
      height: 993px;
    }
      ...
      ul {
        ...
        li {
          ...
          .endTime-Div {
            ...
            align-items: center;
            background: url(../../../assets/common/activity/beginBottom.png) no-repeat;
